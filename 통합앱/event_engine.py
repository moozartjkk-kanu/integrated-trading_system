...
class EventEngine(QObject):
    ...
    # 배치 처리 타이밍 상수
    BATCH_INTERVAL_MS = 5000  # 배치 tick 간격 (5초)
    ...
    def __init__(self, kiwoom_api, config, log_callback=None):
        ...
        # 배치 스케줄러 (5종목씩, 900초 주기로 전체 갱신)
        # TR 호출 제한 준수: 5종목/5초 = 60종목/분 (키움 제한 이내)
        self.batch_scheduler = BatchScheduler(batch_size=5, interval_seconds=900)
    ...
    def _on_batch_timer(self):
        ...
        # 한 바퀴를 모두 돌았다면 다음 전체 사이클 시작까지 대기
        if (
            self._last_cycle_completed_at > 0
            and (time.time() - self._last_cycle_completed_at) < self.batch_scheduler.interval
        ):
            return
    ...
    def _process_current_stock(self):
        ...
        if self.batch_index >= len(self.current_batch):
            self.stock_timer.stop()
            if self.batch_scheduler.current_index == 0:
