...
class TRQueue:
    ...
    def __init__(self, kiwoom_api):
        self.kiwoom = kiwoom_api
        self._queue = deque()  # (tr_func, args, kwargs, callback, tag) 튜플
        self._is_processing = False
        self._process_timer = QTimer()
        self._process_timer.setSingleShot(True)
        self._process_timer.timeout.connect(self._process_next)
    ...
    def enqueue(self, tr_func, callback=None, *args, tag=None, **kwargs):
        """
        TR 호출을 큐에 추가
        ...
        """
        self._queue.append((tr_func, args, kwargs, callback, tag))
    ...
    def _process_next(self):
        ...
        tr_func, args, kwargs, callback, tag = self._queue.popleft()
    ...
    def clear_by_tag(self, tag):
        """지정 태그와 일치하는 대기 TR만 제거"""
        if tag is None:
            return 0

        before = len(self._queue)
        self._queue = deque(item for item in self._queue if item[4] != tag)
        removed = before - len(self._queue)

        if removed > 0:
            self._debug(f"[TR큐] 태그 제거: {tag} ({removed}개)")
...
class KiwoomAPI:
    ...
    def _on_receive_tr_data(self, screen_no, rqname, trcode, record_name, next,
        ...
        # opt10059: 기간별 외국인기관 매매 동향
            ...
            for i in range(min(repeat_cnt, 30)):
                records.append({
                    "date": self.get_comm_data(trcode, rqname, i, "일자"),
                    "foreign": self._get_comm_data_multi(
                        trcode, rqname, i, ["외국인투자자", "외국인", "외인계", "외국계"]
                    ),
                    "institution": self._get_comm_data_multi(
                        trcode, rqname, i, ["기관계", "기관합계", "기관"]
                    ),
                })
    ...
    def get_investor_data(self, code, count=20):
        ...
        self.set_input_value("단위구분", "1000")
        ...
        if records and not any(
            row["foreign"] or row["institution"]
            for row in records
        ):
            self._debug(
                f"[opt10059] code={code} all parsed values are zero; field names or TR inputs should be checked"
            )
    ...
    def get_volume_top_opt10030(self, params: dict, prev_next=0):
        ...
        for key, value in params.items():
            self.set_input_value(key, str(value))
